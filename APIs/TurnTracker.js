...
var TrackerJacker = (function() {
	...
	*/
	...
	var prepareTurnorder = function(turnorder) {
		...
		if (tracker = _.find(turnorder, function(e,i) {if (parseInt(e.id) === -1 && parseInt(e.pr) === -100 && e.custom.match(/Round\s*\d+.\d+/)){return true;}})) {
			// resume logic
		} else {
			turnorder.push({
				id: '-1',
	...
	var updateTurnorderMarker = function(turnorder) {
		...
		if (!!(tracker = _.find(turnorder, function(e,i) {if (parseInt(e.id) === -1 && parseInt(e.pr) === -100 && e.custom.match(/Round\s*\d+.\d+/)){trackerpos = i;return true;}}))) {

			var indicator,
				graphic = findTrackerGraphic(),
				rounds = tracker.custom.substring(tracker.custom.indexOf('Round')).match(/\d+.\d/),
                actions=1;


            if (rounds)
				{
                    actions = parseInt(rounds[0].split(".")[1]);
                    rounds = parseInt(rounds[0].split(".")[0]);
                }
			...
			tracker.custom = indicator + 'Round ' + rounds + '.' + actions;
	...
	var makeTurnDisplay = function(curToken) {
		...
		content += '<div style="background-color: '+design.turncolor+'; font-weight: bold; font-style: italic; border: 2px solid #000; box-shadow: rgba(0,0,0,0.4) 3px 3px; border-radius: 0.5em; text-align: center; min-height: 50px;">'
				...
				+ '<td width="100%">'
					+ (name ? ('C\'est au tour de ' + name) : 'Turn')
				+ '</td>'
				+ '<td width="32px" height="32px">'
	...
	var handleAdvanceTurn = function(turnorder,priororder) {
		...
		if (currentTurn) {
			...
			&& isTracker(currentTurn)) {
                ...
                log(currentTurn.custom);

				var rounds = parseInt(currentTurn.custom.match(/\d+.\d/)[0].split('.')[0]);
				var actions = parseInt(currentTurn.custom.match(/\d+.\d/)[0].split('.')[1]);
				...
				currentTurn.custom = currentTurn.custom.substring(0,currentTurn.custom.indexOf('Round'))
					+ 'Round ' + rounds + '.' + actions;
				announceRound(rounds + '.' + actions);
				turnorder.shift();
	...
	var doResetTurnorder = function(args) {
		...
		if (!turnorder) {
			...
			if(!_.find(turnorder, function(e) {
				if (parseInt(e.id) === -1 && parseInt(e.pr) === -100 && e.custom.match(/Round\s*\d+.\d+/)) {
					e.custom = 'Round ' + initial + '.1';
					return true;
				}
			})) {
				prepareTurnorder();
